...
import chain.ipc.*;
import chain.messages.*;
import chain.timers.*;
import chain.utils.AcceptedValue;
import chain.utils.InstanceState;
import chain.utils.Membership;
import chain.utils.SeqN;
import chain.values.AppOp;
import chain.values.MembershipOp;
import chain.values.NoOpValue;
import chain.values.PaxosValue;
...
public class ZooChainDelayedProto extends GenericProtocol {

    private static final Logger logger = LoggerFactory.getLogger(ZooChainDelayedProto.class);

    public final static short PROTOCOL_ID = 200;
    public final static String PROTOCOL_NAME = "ZooChainProtoDelay";
    ...
    private final Queue<AppOp> waitingAppOps = new LinkedList<>();
    ...
    private final Queue<AppOp> bufferedOps = new LinkedList<>();
    private Map.Entry<Integer, byte[]> receivedState = null;

    private int peerChannel;
    private final ZooChainFront frontend;


    private final EventLoopGroup workerGroup;

    public ZooChainDelayedProto(Properties props, EventLoopGroup workerGroup, ZooChainFront frontend)
    ...
    public void init(Properties props) throws HandlerRegistrationException, IOException {
        ...
        registerRequestHandler(SubmitOpRequest.REQUEST_ID, this::onSubmitOp);
    ...
    private void uponStateTransferMsg(StateTransferMsg msg, Host from, short sourceProto, int channel) {
        ...
        if (state == State.JOINING) {
            ...
            state = State.ACTIVE;
            bufferedOps.forEach(o -> frontend.commit(o.getOpData()));
        }
    }
    ...
    private void becomeLeader(int instanceNumber) {
        ...
        triggerNotification(new LeadNotification(currentSN.getValue().getCounter()));
    ...
    private void ackInstance(int instanceN) {
        ...
        for (int i = highestAcknowledgedInstance + 1; i <= instanceN; i++) {
            ...
            ins.getAttachedReads().forEach(frontend::execRead);
    ...
    public void onSubmitOp(SubmitOpRequest not, short from) {
        if (amQuorumLeader)
            sendNextAccept(new AppOp(not.getOp()));
        else if (supportedLeader().equals(self))
            waitingAppOps.add(new AppOp(not.getOp()));
        else
    ...
    public void onSubmitRead(SubmitReadRequest not, short from) {
        ...
        instances.computeIfAbsent(readInstance, InstanceState::new).attachRead(not.getRequest());
    }
    ...
    private void sendNextAccept(PaxosValue val) {
        ...
        } else if (val.type == PaxosValue.Type.APP_OP) {
            nextValue = val;
        } else
            nextValue = new NoOpValue();
    ...
    private void decideAndExecute(InstanceState instance) {
        ...
        if (instance.acceptedValue.type == PaxosValue.Type.APP_OP)
            if (state == State.ACTIVE)
                frontend.commit(((AppOp) instance.acceptedValue).getOpData());
            else
                bufferedOps.add((AppOp) instance.acceptedValue);
    ...
    private void executeMembershipOp(InstanceState instance) {
        ...
        if (o.opType == MembershipOp.OpType.REMOVE) {
            ...
            if (state == State.ACTIVE) {
                ...
                sendRequest(new GetSnapshotRequest(target, instance.iN), ZooChainFront.PROTOCOL_ID_BASE);
            }
        }
    }
    ...
    private void setNewInstanceLeader(int iN, SeqN sN) {
        ...
        if (amQuorumLeader && !sN.getNode().equals(self)) {
            amQuorumLeader = false;
            triggerNotification(new UnleadNotification());
